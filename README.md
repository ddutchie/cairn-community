...
The catalog of community-contributed **MCP servers**, **HTTP services**, **slash
commands**, **AI providers**, **automations**, and **chat personalities** that
[Cairn](https://github.com/ddutchie/cairn) can browse and install with one click —
no app update required. Cairn fetches the compiled manifests from this
...
| **Chat theme** | A full look for the chat surface — font + typography knobs, background treatment (solid / multi-stop gradient / named pattern), bubble style (filled / glass / outlined) with radius + shadow, and a dark/light palette — picked in **Settings → AI → Tuning**. Pure JSON, rendered data-driven, so new themes arrive without an app update. The five built-ins (Default, Paper, Terminal, Midnight, Aurora) live in the app; this catalog extends them. See [`connectors/themes/README.md`](./connectors/themes/README.md). | Ocean, Glacier, Synthwave |

Skills (larger prompt templates) beyond single-line slash commands may follow in a later release.

> **Four compiled manifests.** MCP servers, HTTP services and slash commands compile into [`manifest.json`](./manifest.json); AI providers compile into a separate [`providers.json`](./providers.json); automations into [`automations.json`](./automations.json); personalities into [`personalities.json`](./personalities.json). All are generated by `scripts/build-manifest.mjs` from the `connectors/` folders — never hand-edit any of them.
...
A **personality** connector (`"kind": "personality"`) is the simplest of all —
no URLs, headers, icons, or auth. It's a set of behavioral rules that Cairn
appends to the chat system prompt to shape the assistant's tone, picked next to
the model selector in chat. These compile into `personalities.json` (a separate
...
```jsonc
{
  ...
  "author": "cairn",
  "version": "1.0.0",
  "category": "Tone & Style",            // one of the personality categories (below)
  "tags": ["tone", "brevity"],           // freeform search keywords
  "blurb": "Short, plain words. Answer first, no fluff.",
  "brandColor": "#9ca3af",               // optional — shown as a dot in the picker
  "homepage": "https://github.com/JuliusBrussee/caveman",  // optional — attribution/source
  "definition": {
    "name": "Caveman",                   // display name in the picker
    "description": "Blunt, minimal, plain-English answers.",
    "prompt": "Speak in short, simple sentences.\nUse plain words.\nGive the answer or result first.\n..."
  }
}
```

Personality `definition.prompt` is **appended verbatim** to the base Cairn system
prompt (which opens "You are the Cairn AI assistant…"). Rules for writing one:

- **Behavioral rules, not labels.** "Give the answer or result first" beats "be concise".
- **Never open with `You are …`.** A personality is a style *layer* on the existing
  assistant identity — its own "You are Grug Assistant" would contradict the base
  prompt. CI rejects prompts starting with "You are".
- **20+ characters minimum** — a one-liner adds nothing to the system prompt.
- **Name follows the source.** When a personality is based on a named skill or
  repo (like the Caveman Claude skill), use that name (`Caveman`, not `Grug`) so
  the picker row matches its `homepage` attribution. Inventive names are fine for
  original creations.
- `prompt` is untrusted community text: Cairn shows it **in full before install**
  and it is never executed, but it does reach the LLM. Keep it descriptive and
  benign.

Personality categories (the Browse chips): **Tone & Style · Planning & Strategy ·
Execution · Learning · Technical · Critique & Review · Assistant · Fun & Playful**.
`category` must be one of these (personalities use their own set, distinct from
the connector categories). `name` must be unique among personalities
(case-insensitive). `homepage` (optional) is a good place to credit the source
technique or skill a personality is based on.
...
- **Personalities**: `definition.prompt` must be non-empty, 20+ chars, and must NOT start with `You are` (a style layer, not a new identity — see above). `category` must be one of the personality categories. They compile into `personalities.json`, not `manifest.json`.
